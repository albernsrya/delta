...
from delta.ml.predict import ImagePredictor, LabelPredictor
...
def test_train_validate(identity_config, binary_identity_tiff_filenames,
                        tmp_path):
    config_reset()
    train_config = tmp_path / "config.yaml"
    with open(train_config, "w") as f:
        ...
        train:
          ...
          callbacks:
            - ExponentialLRScheduler:
                start_epoch: 2
        """ % (
            binary_identity_tiff_filenames[0][0],
            binary_identity_tiff_filenames[1][0],
        ))
    args = "delta train --config %s --config %s" % (identity_config,
                                                    train_config)
    main(args.split())
