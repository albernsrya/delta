...
from delta.ml import io, predict, train
...
def test_pretrained(dataset, ae_dataset):
    ...
    def autoencoder_fn():
        ...
        conv1 = keras.layers.Conv2D(filters=16,
                                    kernel_size=3,
                                    activation="relu",
                                    padding="same")(inputs)
        down_samp1 = keras.layers.MaxPooling2D((2, 2), padding="same")(conv1)
        encoded = keras.layers.Conv2D(filters=8,
                                      kernel_size=3,
                                      activation="relu",
                                      padding="same")(down_samp1)

        # at this point the representation is (4, 4, 8) i.e. 128-dimensional

        up_samp1 = keras.layers.UpSampling2D((2, 2))(encoded)
        conv4 = keras.layers.Conv2D(16, (3, 3),
                                    activation="relu",
                                    padding="same")(up_samp1)
        decoded = keras.layers.Conv2D(1, (3, 3),
                                      activation="sigmoid",
                                      padding="same")(conv4)
    ...
    def model_fn():
        ...
        conv1 = keras.layers.Conv2D(16, (3, 3),
                                    activation="relu",
                                    padding="same")(up_samp1)
        output = keras.layers.Conv2D(2, (3, 3),
                                     activation="softmax",
                                     padding="same")(conv1)
