...
from delta.imagery import \
    imagery_dataset  # pylint: disable=wrong-import-position
...
), "For speed of command line tool, tensorflow should not be imported by config!"
...
def generate_tile(width=32, height=32, blocks=50):
    ...
    for _ in range(blocks):
        ...
        while ((image[x + 2, y - 2:y + 2, 0] > 0.0).any()
               or (image[x + 2, y - 2:y + 2, 0] > 0.0).any()
               or (image[x - 1:x + 1, y - 2, 0] > 0.0).any()
               or (image[x - 1:x + 1, y + 2, 0] > 0.0).any()):
            x = random.randint(2, width - 3)
            y = random.randint(2, height - 3)
        image[x + 1, y - 1, 0] = 1.0
...
def binary_identity_tiff_filenames():
    ...
    label = np.random.randint(
        0, 2, (128, 128), np.uint8)  # pylint: disable=no-member
