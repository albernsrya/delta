...
def merge_images(input_paths, output_file):
    ...
    for i, path in enumerate(input_paths):
        if i == 0:
            ref_image = gdal.Open(
                path
            )  # the first input image is considered as the reference and will be compared
            ref_path = (
                path  # with all the other images to make sur that merging is possible
            )
        else:
            ...
            if not images_equivalent(
                    ref_image, gdal.Open(path), raster_size=False,
                    coverage=False):
                print(
                    "Differences between %s and %s prevent merging. Aborted." %
...
def match_images(input_paths, output_path, target_path=None):
    ...
    if (
            target_path is None
    ):  # If no target is given, the first input is considered as the target
        if len(input_paths) == 1:
            print(
                "Only one input was given. Need a target or another input to match."
            )
            return
        target_path = input_paths[0]
        target_name = os.path.split(input_paths[0])[1].split(".")[0]
        input_paths = np.delete(input_paths, 0)
        print("No target was given. Replacing it with the first input: {}".
              format(target_name))
    else:
        target_name = os.path.split(target_path)[1].split(".")[0]

    data_match = gdal.Open(target_path)
    geo_match = (data_match.GetGeoTransform()
                 )  # target image characteristics used as reference
    srs_match = data_match.GetSpatialRef()
    corner_match_dict = gdal.Info(data_match,
                                  format="json")["cornerCoordinates"]
    corner_match_arr = [
        corner_match_dict.get(key) for key in corner_match_dict.keys()
        if key in ("upperLeft", "lowerRight")
    ]

    # sets the biggest shared region to the whole image
    biggest_common_bounds = (
        corner_match_arr[0][0],
        corner_match_arr[1][1],
        corner_match_arr[1][0],
    ...
    try:
        ...
        print(
            "A directory already exists for images matched with {}. Old files can be replaced.\n"
            .format(target_name))
    ...
    for i, path in enumerate(input_paths):
        ...
        output_file[i] = os.path.join(new_path,
                                      input_name + "_match." + file_ext)
        ...
        if not images_equivalent(
            ...
            overlap_geo, overlap_pix_match, overlap_pix_src = compute_overlap_region(
                data_match, img)

            # No overlap between the images then process is stopped
            if (overlap_geo is None or overlap_pix_match is None
                    or overlap_pix_src is None):
                print(
                    "Can't match {} to {} because they don't overlap geographically."
                    .format(path, target_name))
                return
            # Updates the shared region boundaries
            biggest_common_bounds = (
                max(biggest_common_bounds[0], overlap_geo[0][0]),
            ...
            if (biggest_common_bounds[0] > biggest_common_bounds[2]
                ...
                print(
                    "Some inputs do not have a common overlapping region with the target. Matching them is not "
            ...
            print(
                "{} and {} cover the same region (left corner:[{}, {}], right corner:[{}, {}])."
                .format(
                    input_name,
                    target_name,
                    corner_match_arr[0][0],
    ...
    print("\nCommon overlapping region to all the images: {}".format(
        biggest_common_bounds))

    for i, path in enumerate(input_paths):

        print("Matching {} to {} in the overlapping region {}.".format(
            path, target_name, biggest_common_bounds))
    ...
    if target_need_crop:
        crop_target_path = os.path.join(new_path,
                                        target_name + "_crop." + file_ext)

        print(
            "Cropping the overlapping region from the target so that the images cover the same region."
        )
        ...
        gdal.Translate(
            ...
            target_path,
            projWin=[
                biggest_common_bounds[0],
                biggest_common_bounds[3],
                biggest_common_bounds[2],
                biggest_common_bounds[1],
            ],
        )
...
def images_equivalent(
    ...
    assert isinstance(db_ref, gdal.Dataset) and isinstance(
        db_new, gdal.Dataset), "Inputs must be gdal datasets."
    ...
    if projection:
        if db_new.GetProjection() != db_ref.GetProjection():
            ...
            if not osr.SpatialReference(db_ref.GetProjection()).IsSame(
                    osr.SpatialReference(db_new.GetProjection())):
                return False
...
def compute_overlap_region(db_ref, db_new):
    ...
    cc_val_ref = [
        cornerCoord_ref.get(key) for key in cornerCoord_ref.keys()
        if key != "center"
    ]
    [ul_ref, dummy, lr_ref, _] = cc_val_ref

    cc_val_new = [
        cornerCoord_new.get(key) for key in cornerCoord_new.keys()
        if key != "center"
    ]
    ...
    if cc_val_ref == cc_val_new:
        print(
            "The images cover the same region (left corner:[{}, {}], right corner:[{}, {}])."
    ...
    overlap_corners = [
        [max(ul_ref[0], ul_new[0]),
         min(ul_ref[1], ul_new[1])],
        [min(lr_ref[0], lr_new[0]),
    ...
    if (overlap_corners[0][0] > overlap_corners[1][0]
            or overlap_corners[0][1] < overlap_corners[1][1]):
        print("The two regions represented by the images don't overlap.")
        return None, None, None
    print(
        "Found an overlapping regions (left corner:[{}, {}], right corner:[{}, {}])."
        .format(
            overlap_corners[0][0],
            overlap_corners[0][1],
            overlap_corners[1][0],
    ...
    # for both images
    col_ul_ref, row_ul_ref = geo2pix(db_ref.GetGeoTransform(),
                                     overlap_corners[0][0],
                                     overlap_corners[0][1])
    col_lr_ref, row_lr_ref = geo2pix(db_ref.GetGeoTransform(),
                                     overlap_corners[1][0],
                                     overlap_corners[1][1])

    col_ul_new, row_ul_new = geo2pix(db_new.GetGeoTransform(),
                                     overlap_corners[0][0],
                                     overlap_corners[0][1])
    col_lr_new, row_lr_new = geo2pix(db_new.GetGeoTransform(),
                                     overlap_corners[1][0],
                                     overlap_corners[1][1])

    return (
        overlap_corners,
        [[row_ul_ref, col_ul_ref], [row_lr_ref, col_lr_ref]],
        [[row_ul_new, col_ul_new], [row_lr_new, col_lr_new]],
    )


def fill_no_data_value(
    input_path,
    interpolation_distance=16,
    ...
    if create_new_file:
        if output_file is None:
            print(
                "A path for the output file is mandatory if create_new_file is True. Because none was given, "
                "the new file will be saved at the same location as the input."
            )
            output_file = os.path.join(input_head,
                                       input_name + "_filled." + input_ext)
    ...
    for i in range(0, new_db.RasterCount):
        ...
        if band.GetMaskFlags() == gdal.GMF_ALL_VALID:
            print(
                "All pixels of band {} of {} have a value. No need to fill it."
                .format(i + 1, input_name))
            doFill[i] = False

    # If all bands don't need to be filled then the process is stopped
    if np.unique(doFill).size == 1 and not np.unique(doFill)[0]:
        print("All pixels of {} are valid. No filling will be applied.".format(
            input_name))
        return 0
    ...
    print("Filled successfully the pixels without data of {}.".format(
        input_tail))
    return 1
...
def compute_normalized_dsm(dsm_file,
                           dem_file,
                           apply_blur=False,
                           output_file=None):
    """
    ...
    if output_file is None:
        print(
            "No output file was given. The nDSM will be stored where the DSM is: {}"
    ...
    assert (os.system(
        "gdal_cacl.py --calc=A-b -A %s -B %s --outfile %s --quiet" %
