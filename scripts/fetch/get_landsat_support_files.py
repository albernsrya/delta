...
def fetch_images(ll_coord, ur_coord, output_folder, options):
    ...
    if options.force_login or (not api._get_api_key(None)):  # pylint: disable=W0212
        ...
        dummy_result = api.login(options.user_ee,
                                 options.password_ee,
                                 save=True,
                                 catalogId=CATALOG)
...
def main(argsIn):
    ...
    if (options.user_ee and options.password_ee and options.user_urs
            and options.password_urs):
        ...
        print(
            "user and password inputs not provided, skipping label download step."
        )
    ...
    cmd = ("gdalbuildvrt -vrtnodata None " + merge_path + " " +
           os.path.join(options.label_folder, "*.tif"))
    print(cmd)
    os.system(cmd)
