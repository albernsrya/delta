...
def ae_convert(data):
    r = np.clip((data[:, :, [4, 2, 1]] * np.float32(100.0)), 0.0,
...
def main(options):
    ...
    for (i, path) in enumerate(images):
        ...
        if options.autoencoder:
            ...
            predictor = predict.LabelPredictor(
                model,
                ts,
                output_image,
                True,
        ...
        if labels:
            ...
            for j in range(cm.shape[0]):
                print(
                    "%s--- Precision: %.2f%%    Recall: %.2f%%      Pixels: %d / %d"
                    % (
            ...
            save_confusion(
                cm,
                map(lambda x: x.name, config.dataset.classes),
                "confusion_" + base_name + ".pdf",
            )

        if options.autoencoder:
            write_tiff(
                "orig_" + base_name + ".tiff",
                image.read()
