...
def setup_classify(subparsers):
    sub = subparsers.add_parser("classify",
                                help="Classify images given a model.")
    ...
    sub.add_argument(
        ...
        dest="prob",
        action="store_true",
        help="Save image of class probabilities.",
    )
    sub.add_argument(
        "--autoencoder",
        dest="autoencoder",
        action="store_true",
        help="Classify with the autoencoder.",
    )
    sub.add_argument(
        "--no-colormap",
        dest="noColormap",
    ...
    sub.add_argument("--validation",
                     dest="validation",
                     help="Classify validation images instead.")
...
def setup_train(subparsers):
    sub = subparsers.add_parser("train",
                                help="Train a task-specific classifier.")
    config.setup_arg_parser(sub)
    sub.add_argument("--autoencoder",
                     action="store_true",
                     help="Train autoencoder (ignores labels).")
    sub.add_argument(
        "--resume", help="Use the model as a starting point for the training.")
    sub.add_argument("model",
                     nargs="?",
                     default=None,
                     help="File to save the network to.")
...
def setup_mlflow_ui(subparsers):
    sub = subparsers.add_parser(
        "mlflow_ui",
        help="Launch mlflow user interface to visualize run history.")
