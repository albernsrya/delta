...
from delta.imagery.imagery_dataset import AutoencoderDataset, ImageryDataset

from .config_parser import (config_callbacks, loss_from_dict, metric_from_dict,
                            optimizer_from_dict)
from .io import print_network, save_model
...
def _devices(num_gpus):
    ...
    if num_gpus == 0:
        ...
        assert len(
            devs) >= num_gpus, "Requested %d GPUs with only %d available." % (
                num_gpus,
                len(devs),
        )
        if num_gpus > 0:
    ...
    if tc.validation:
        if tc.validation.from_training:
            ...
            if not vimg:
                ...
                if vlabel:
                    vimagery = ImageryDataset(
                        vimg,
                        vlabel,
                        ids.output_shape(),
                        ids.chunk_shape(),
                        tile_shape=ids.tile_shape(),
                        stride=ids.stride(),
                        tile_overlap=ids.tile_overlap(),
                    )
                else:
                    vimagery = AutoencoderDataset(
                        vimg,
                        ids.chunk_shape(),
                        tile_shape=ids.tile_shape(),
                        stride=ids.stride(),
...
def _log_mlflow_params(model, dataset, training_spec):
    ...
    mlflow.log_param(
        "Model - Parameters - Non-Trainable",
...
class _MLFlowCallback(tf.keras.callbacks.Callback):
    ...
    def on_train_batch_end(self, batch, logs=None):
        ...
        if (config.mlflow.checkpoints.frequency()
                and batch % config.mlflow.checkpoints.frequency() == 0):
...
def _build_callbacks(model, dataset, training_spec):
    ...
    if config.tensorboard.enabled():
        ...
        tcb = tf.keras.callbacks.TensorBoard(
            log_dir=tb_dir,
            update_freq="epoch",
            histogram_freq=1,
            write_images=True,
...
def _compile_helper(model, training_spec):
    model.compile(
        optimizer=optimizer_from_dict(training_spec.optimizer),
...
def compile_model(model_fn, training_spec, resume_path=None):
    ...
    with training_spec.strategy.scope():
        ...
        assert isinstance(
            model,
            tf.keras.models.Model), "Model is not a Tensorflow Keras model"
    ...
    assert (len(output_shape) == 2 or output_shape[1]
            == output_shape[2]), "Output from network is not chunked"
...
def train(model_fn, dataset: ImageryDataset, training_spec, resume_path=None):
    ...
    assert (model.input_shape[3] == dataset.num_bands()
            ), "Number of bands in model does not match data."
    # last element differs for the sparse metrics
    assert model.output_shape[1:-1] == dataset.output_shape()[:-1] or (
        model.output_shape[1] is
        None), "Network output shape %s does not match label shape %s." % (
            model.output_shape[1:],
            dataset.output_shape()[:-1],
    )
    ...
    try:
        ...
        if (training_spec.steps is None) or (training_spec.steps > 0):
            ...
            history = model.evaluate(
                validation,
                steps=training_spec.validation.steps,
                callbacks=callbacks,
                verbose=1,
            )
