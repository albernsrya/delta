...
from delta.imagery.imagery_config import (ImageSet, ImageSetConfig,
                                          load_images_labels)
...
class NetworkConfig(config.DeltaConfigComponent):
    ...
    def _load_dict(self, d: dict, base_dir):
        ...
        if "layers" in d and d["layers"] is not None:
            ...
            if "layers" in d and d["layers"] is not None:
                raise ValueError(
                    "Specified both yaml file and layers in model.")
            yaml_file = d["yaml_file"]
            resource = os.path.join("config", yaml_file)
            if not os.path.exists(yaml_file) and pkg_resources.resource_exists(
                    "delta", resource):
                yaml_file = pkg_resources.resource_filename("delta", resource)
            if not os.path.exists(yaml_file):
                raise ValueError("Model yaml_file does not exist: " +
                                 yaml_file)
...
def _validate_stride(stride, _):
    ...
    assert isinstance(stride[0], int) and isinstance(
        stride[1], int), "Stride must be integer."
...
class TrainingConfig(config.DeltaConfigComponent):
    ...
    def __init__(self):
        ...
        self.register_field("loss", (str, dict), None, None,
                            "Keras loss function.")
        self.register_field("metrics", list, None, None,
                            "List of metrics to apply.")
        self.register_field(
            "steps",
            int,
            None,
            config.validate_non_negative,
            "Batches to train per epoch.",
        )
        self.register_field("optimizer", (str, dict), None, None,
                            "Keras optimizer to use.")
        self.register_field("callbacks", list, "callbacks", None,
                            "Callbacks used to modify training")
    ...
    def spec(self) -> TrainingSpec:
        ...
        if not self.__training:
            ...
            if not from_training:
                (vimg, vlabels) = (
                    self._components["validation"].images(),
                    self._components["validation"].labels(),
                )
            validation = ValidationSet(vimg, vlabels, from_training, vsteps)
            self.__training = TrainingSpec(
                batch_size=self._config_dict["batch_size"],
...
class MLFlowConfig(config.DeltaConfigComponent):
    ...
    def __init__(self):
        ...
        self.register_field("uri", str, None, None,
        ...
        self.register_field("experiment_name", str, "experiment", None,
                            "Experiment name in MLFlow.")

        self.register_arg("enabled",
                          "--disable-mlflow",
                          action="store_const",
                          const=False,
                          type=None)
...
class TensorboardConfig(config.DeltaConfigComponent):
    ...
    def __init__(self):
        super().__init__()
        self.register_field("enabled", bool, "enabled", None,
                            "Enable Tensorboard.")
        self.register_field("dir", str, None, None,
...
def register():
    ...
    """
    config.config.general.register_field("gpus", int, "gpus", None,
