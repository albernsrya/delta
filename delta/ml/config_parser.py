...
class _LayerWrapper:
    ...
    def sub_layer(self, name):
        assert self._sub_layers, "Layer %s does not support sub-layers." % (
            self.layer.name)
        assert name in self._sub_layers, ("Layer %s not found in " %
                                          (name)) + str(self._sub_layers)
    ...
    def output_tensor(self):
        ...
        for k in self._inputs:
            ...
            if input_layer not in self._all_layers:
                raise ValueError("Input layer " + str(input_layer) +
                                 " not found.")
            self._all_layers[input_layer].output_tensor(
            )  # compute it if it hasn't been
            cur = self._all_layers[input_layer].sub_layer(k[len(parts[0]) +
                                                            1:])
...
def _apply_params(model_dict, exposed_params):
    ...
    def recursive_dict_list_apply(d, func):
        ...
        if isinstance(d, list):
            return list(
                map(functools.partial(recursive_dict_list_apply, func=func),
                    d))
        if isinstance(d, str):
            return func(d)
        return d
    ...
    if first_layer_type != "Input" and "input" not in layer_list[0][
            first_layer_type]:
        model_dict_copy["layers"] = [{
            "Input": {
                "shape": params["in_shape"]
            }
        }] + layer_list
...
def model_from_dict(
        model_dict: dict,
        exposed_params: dict) -> Callable[[], tensorflow.keras.models.Model]:
    """
    Construct a model.
...
def _parse_str_or_dict(spec, type_name):
    ...
    if isinstance(spec, dict):
        assert len(
            spec.keys()) == 1, "Only one %s may be specified." % (type_name)
...
def loss_from_dict(
        loss_spec: Union[dict, str]) -> tensorflow.keras.losses.Loss:
    """
...
def metric_from_dict(
        metric_spec: Union[dict, str]) -> tensorflow.keras.metrics.Metric:
    """
    Construct a metric.
    ...
    if mc is None:
        try:
            ...
            raise ValueError("Unknown metric %s." %
                             (name))  # pylint:disable=raise-missing-from
    if isinstance(mc, type) and issubclass(mc,
                                           tensorflow.keras.metrics.Metric):
        mc = mc(**params)
    return mc


def optimizer_from_dict(
        spec: Union[dict, str]) -> tensorflow.keras.optimizers.Optimizer:
    """
...
def callback_from_dict(
        callback_dict: Union[dict,
                             str]) -> tensorflow.keras.callbacks.Callback:
    """
    ...
    tensorflow.keras.callbacks.Callback
        The callback object.
    """
    assert (len(callback_dict.keys()) == 1
            ), f"Error: Callback has more than one type {callback_dict.keys()}"
...
def config_callbacks() -> List[tensorflow.keras.callbacks.Callback]:
    ...
    if not config.train.callbacks() is None:
        return [
            callback_from_dict(callback)
            for callback in config.train.callbacks()
        ]
    return []


def config_model(
        num_bands: int) -> Callable[[], tensorflow.keras.models.Model]:
    """
    Returns
    -------
    Callable[[], tensorflow.keras.models.Model]
