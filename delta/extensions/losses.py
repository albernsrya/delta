...
def ms_ssim_mse(y_true, y_pred):
    ...
    """
    return ms_ssim(y_true, y_pred) + K.mean(
        K.mean(tensorflow.keras.losses.MSE(y_true, y_pred), -1), -1)
    ...
    def __init__(self, mapping, name=None):
        ...
        if isinstance(mapping, list):
            ...
            if np.isscalar(entry):
                ...
                map_list = np.zeros(
                    (len(config.dataset.classes) + 1, len(entry)))
            assert len(mapping) == len(
                config.dataset.classes
            ), "Must specify all classes in loss mapping."
            for k in mapping:
                i = config.dataset.classes.class_id(k)
                if isinstance(mapping[k], (int, float)):
                    map_list[i] = mapping[k]
                else:
                    assert (len(mapping[k]) == map_list.shape[1]
                            ), "Mapping entry wrong length."
...
class MappedCategoricalCrossentropy(MappedLoss):
    ...
    def call(self, y_true, y_pred):
        ...
        true_convert = tf.gather(self._lookup,
...
class MappedBinaryCrossentropy(MappedLoss):
    ...
    def call(self, y_true, y_pred):
        true_convert = tf.gather(self._lookup,
                                 tf.cast(y_true, tf.int32),
                                 axis=None)
        return tensorflow.keras.losses.binary_crossentropy(
            true_convert, y_pred)
