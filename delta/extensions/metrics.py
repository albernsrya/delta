...
class SparseRecall(SparseMetric):  # pragma: no cover
    ...
    def __init__(self,
        ...
        self._true_positives = self.add_weight("true_positives",
                                               initializer="zeros")
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):  # pylint: disable=unused-argument, arguments-differ
        ...
        true_positives = tf.math.reduce_sum(tf.cast(true_positives,
                                                    tf.float32))
...
class SparseBinaryAccuracy(SparseMetric):  # pragma: no cover
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):  # pylint: disable=unused-argument, arguments-differ
        ...
        true_positives = tf.math.logical_and(right_class, right_class_pred)
        false_negatives = tf.math.logical_and(
            tf.math.logical_not(right_class),
            tf.math.logical_not(right_class_pred))
        if self._nodata_id:
        ...
        true_positives = tf.math.reduce_sum(tf.cast(true_positives,
                                                    tf.float32))
        false_negatives = tf.math.reduce_sum(
            tf.cast(false_negatives, tf.float32))
