...
class WorldviewImage(tiff.TiffImage):
        ...
        if ext == ".zip":  # Need to unpack
            ...
            tif_names = list(
                filter(lambda x: x.lower().endswith(".tif"),
                       zip_file.namelist()))
            assert len(
                tif_names) > 0, f"Error: no tif files in the file {paths}"
            assert (len(tif_names) == 1
                    ), f"Error: too many tif files in {paths}: {tif_names}"
    ...
    def __parse_meta_file(self, meta_path):
        ...
        data = {"ABSCALFACTOR": [], "EFFECTIVEBANDWIDTH": []}
...
def _apply_toa_radiance(data, _, bands, factors, widths):
    ...
    for b in bands:
        f = factors[b]
        w = widths[b]
        buf[:, :, b] = np.where(data[:, :, b] > 0, (data[:, :, b] * f) / w,
                                OUTPUT_NODATA)
    return buf
...
def toa_preprocess(image, calc_reflectance=False):
    ...
    if not calc_reflectance:
        user_function = functools.partial(_apply_toa_radiance,
                                          factors=image.scale(),
                                          widths=image.bandwidth())
    else:
        raise Exception(
            "TODO: WV reflectance calculation is not fully implemented!")
