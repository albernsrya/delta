...
class TiffImage(delta_image.DeltaImage):
    ...
    def __init__(self, path, nodata_value=None):
        ...
        for i, h in enumerate(self._handles):
            if (h.RasterXSize != self._handles[0].RasterXSize
                    or h.RasterYSize != self._handles[0].RasterYSize):
                raise Exception("Images %s and %s have different sizes!" %
                                (self._paths[0], self._paths[i]))
            for j in range(h.RasterCount):
                self._band_map.append(
                    (i, j + 1))  # gdal uses 1-based band indexing
    ...
    def _read(self, roi, bands, buf=None):
        ...
        if buf is None:
            buf = np.zeros(shape=(num_bands, roi.width(), roi.height()),
                           dtype=self.dtype())
        for i, b in enumerate(bands):
            ...
            if s != (roi.width(), roi.height()):
                raise IOError(
                    "Buffer shape should be (%d, %d) but is (%d, %d)!" %
                    (roi.width(), roi.height(), s[0], s[1]))
            band_handle.ReadAsArray(roi.min_y,
                                    roi.min_x,
                                    roi.height(),
                                    roi.width(),
                                    buf_obj=buf[i, :, :])
...
def write_tiff(output_path: str, data: np.ndarray, metadata: dict = None):
    ...
    with _TiffWriter(
            ...
            data_type=data_type,
            metadata=metadata,
            tile_width=min(TILE_SIZE, data.shape[0]),
            tile_height=min(TILE_SIZE, data.shape[1]),
    ) as writer:
        for x in range(0, data.shape[0], TILE_SIZE):
            for y in range(0, data.shape[1], TILE_SIZE):
                ...
                if len(data.shape) < 3:
                    writer.write_block(
                        data[x:x + TILE_SIZE, y:y + TILE_SIZE],
                        block[0],
                        block[1],
                        0,
                    )
                    ...
                    for b in range(num_bands):
                        writer.write_block(
                            data[x:x + TILE_SIZE, y:y + TILE_SIZE, b],
                            block[0],
                            block[1],
                            b,
                        )
...
class _TiffWriter:
    ...
    def __init__(
        ...
        height,
        num_bands=1,
        data_type=gdal.GDT_Byte,  # pylint:disable=too-many-arguments
        tile_width=256,
        tile_height=256,
        nodata_value=None,
        metadata=None,
        ...
        options += [
            "BLOCKXSIZE=" + str(self._tile_width),
        ...
        self._handle = driver.Create(path, height, width, num_bands, data_type,
                                     options)
        if not self._handle:
    ...
    def write_block(self, data, block_x, block_y, band=0):
        ...
        is_edge_block = (block_x == num_tiles[0] - 1) or (block_y
                                                          == num_tiles[1] - 1)
        ...
        if is_edge_block:  # Data must fit inside the image size
            ...
            if (data.shape[0] != self._tile_width) or (data.shape[1] !=
                                                       self._tile_height):
                raise Exception("Error: Data block size is " +
                                str(data.shape) +
        ...
        gdal_band.WriteArray(data, block_y * self._tile_height,
                             block_x * self._tile_width)
...
class TiffWriter(delta_image.DeltaImageWriter):
    ...
    def initialize(self, size, numpy_dtype, metadata=None, nodata_value=None):
        ...
        TILE_SIZE = 256
        self._tiff_w = _TiffWriter(
            self._filename,
            size[0],
            size[1],
            num_bands=size[2],
            data_type=_numpy_dtype_to_gdal_type(numpy_dtype),
            metadata=metadata,
            nodata_value=nodata_value,
            tile_width=min(TILE_SIZE, size[0]),
            tile_height=min(TILE_SIZE, size[1]),
        )
