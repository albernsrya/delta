            ...
            for tag in DESIRED_TAGS:
                ...
                if t in line:  # TODO: Better to do regex here
                    ...
                    try:
                        ...
                        band = int(
                            name.split("_")[-1]) - 1  # One-based to zero-based
...
def _find_mtl_file(folder):
    ...
    if len(meta_files) > 1:
        raise Exception("Error: Too many MTL files in ", folder,
...
class LandsatImage(tiff.TiffImage):
    ...
    def _prep(self, paths):
        ...
        bands_to_use = (_get_landsat_bands_to_use(self._sensor)
                        if self._bands is None else self._bands)
...
def _apply_toa_radiance(data, _, bands, factors, constants):
    ...
    for b in bands:
        f = factors[b]
        c = constants[b]
        buf[:, :, b] = np.where(data[:, :, b] > 0, data[:, :, b] * f + c,
                                OUTPUT_NODATA)
    return buf
...
def _apply_toa_reflectance(data, _, bands, factors, constants, sun_elevation):
    ...
    for b in bands:
        ...
        c = constants[b]
        se = sun_elevation[b]
        buf[:, :, b] = np.where(data[:, :, b] > 0,
                                (data[:, :, b] * f + c) / math.sin(se),
                                OUTPUT_NODATA)
    return buf
...
def toa_preprocess(image, calc_reflectance=False):
    ...
    if calc_reflectance:
        if image.k1_constant() is None:
            user_function = functools.partial(
                _apply_toa_reflectance,
                factors=image.reflectance_mult(),
                constants=image.reflectance_add(),
                sun_elevation=math.radians(image.sun_elevation()),
            )
        else:
            user_function = functools.partial(
                _apply_toa_temperature,
                factors=image.radiance_mult(),
                constants=image.radiance_add(),
                k1=image.k1_constant(),
