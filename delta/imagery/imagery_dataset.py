...
class ImageryDataset:  # pylint: disable=too-many-instance-attributes
    ...
    def _list_tiles(self, i):  # pragma: no cover
        ...
        if self._resume_mode:
            ...
            if log_path:
                if config.general.verbose():
                    print("get_image_tile_list for index " + str(i) + " -> " +
                          file_path)
                ...
                if need_to_check and (count > config.train.resume_cutoff()):
                    if config.general.verbose():
                        print("Skipping index " + str(i) +
                              " tile gen with count " + str(count) + " -> " +
                              file_path)
                    return []
                if config.general.verbose():
                    print("Computing tile list for index " + str(i) +
        ...
        if (
                self._labels
        ):  # If we have labels make sure they are the same size as the input images
            label = self._labels.load(i)
            if label.size() != img.size():
                raise AssertionError("Label file " + self._labels[i] +
                                     " with size " + str(label.size()) +
                                     " does not match input image size of " +
                                     str(img.size()))
        ...
        if self._chunk_shape:
            ...
            return img.tiles(
                ...
                min_shape=self._chunk_shape,
                overlap_shape=(self._chunk_shape[0] - 1,
                               self._chunk_shape[1] - 1),
                by_block=True,
            )
        return img.tiles(
            (tile_shape[0], tile_shape[1]),
    ...
    def _tile_generator(self, i, is_labels):  # pragma: no cover
        ...
        image.set_preprocess(
            None)  # parallelize the preprocessing, not in disk i/o threadpool
        ...
        for (c, (rect, sub_tiles)) in enumerate(tiles):
            ...
            if c + 1 < len(tiles):
                ...
                next_buf = self._iopool.submit(
                    (lambda x: (lambda: image.read(tiles[x + 1][0])))(c))
            if cur_buf is None:
                continue
            buf = cur_buf.result()
            ...
            for s in sub_tiles:
                if preprocess:
                    ...
                    yield preprocess(buf[s.min_x:s.max_x, s.min_y:s.max_y, :],
                                     t, bands)
                else:
    ...
    def _load_images(self, is_labels, data_type):
        ...
        r = r.shuffle(1000, seed=0, reshuffle_each_iteration=True
                      )  # shuffle same way for labels and non-labels
        self._access_counts[1 if is_labels else 0] = np.zeros(
            len(self._images), np.uint8)  # count epochs for random

        # different seed for each image, use ge

        def gen_func(x):
            return tf.data.Dataset.from_generator(
                functools.partial(self._tile_generator, is_labels=is_labels),
        ...
        return r.interleave(
            gen_func,
            cycle_length=config.io.interleave_images(),
    ...
    def _chunk_image(self, image):  # pragma: no cover
        ...
        strides = [
            ...
            self._stride[1],
            1,
        ]  # Spacing between chunk starts
        rates = [1, 1, 1, 1]
        result = tf.image.extract_patches(tf.expand_dims(image, 0),
                                          ksizes,
                                          strides,
                                          rates,
                                          padding="VALID")
        ...
        result = tf.reshape(
            result,
            [-1, self._chunk_shape[0], self._chunk_shape[1], self._num_bands])
    ...
    def _reshape_labels(self, labels):  # pragma: no cover
        ...
        labels = tf.image.crop_to_bounding_box(labels, w, h,
                                               tf.shape(labels)[0] - 2 * w,
        ...
        rates = [1, 1, 1, 1]
        labels = tf.image.extract_patches(tf.expand_dims(labels, 0),
                                          ksizes,
                                          strides,
                                          rates,
                                          padding="VALID")
        result = tf.reshape(labels,
                            [-1, self._output_shape[0], self._output_shape[1]])
        return result
    ...
    def labels(self):
        ...
        if self._chunk_shape or self._output_shape:
            label_set = label_set.map(
                self._reshape_labels,
                num_parallel_calls=tf.data.experimental.AUTOTUNE)  # pylint: disable=C0301
            if self._chunk_shape:
    ...
    def dataset(self, class_weights=None):
        ...
        if self._labels.nodata_value() is not None:
            ds = ds.filter(lambda x, y: tf.math.reduce_any(
                tf.math.not_equal(y, self._labels.nodata_value())))
        if class_weights is not None:
            class_weights.append(0.0)
            lookup = tf.constant(class_weights)
            ds = ds.map(
                lambda x, y:
                (x, y, tf.gather(lookup, tf.cast(y, tf.int32), axis=None)),
                num_parallel_calls=config.io.threads(),
            )
        return ds
    ...
    def set_chunk_output_shapes(self, chunk_shape, output_shape):
        ...
        if chunk_shape:
            ...
            assert ((chunk_shape[0] % 2) == (chunk_shape[1] % 2) ==
                    (output_shape[0] % 2) ==
                    (output_shape[1] %
                     2)), "Chunk and output shapes must both be even or odd."
        if output_shape:
            assert (len(output_shape) == 2 or len(output_shape)
                    == 3), "Output must be two or three dimensional."
    ...
    def input_shape(self):
        ...
        """
        if self._chunk_shape:
            return (self._chunk_shape[0], self._chunk_shape[1],
                    self._num_bands)
    ...
    def output_shape(self):
        ...
        if self._output_shape:
            return (self._output_shape[0], self._output_shape[1],
                    self._output_dims)
