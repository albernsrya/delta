...
from delta.config import (DeltaConfigComponent, config, validate_path,
                          validate_positive)
...
def __find_images(conf, matching_images=None, matching_conf=None):
    ...
    if conf["files"]:
        assert (conf["file_list"] is None and conf["directory"] is None
                ), "Only one image specification allowed."
        images = conf["files"]
        for (i, im) in enumerate(images):
            images[i] = os.path.normpath(im)
    elif conf["file_list"]:
        assert conf[
            "directory"] is None, "Only one image specification allowed."
        ...
        if not os.path.exists(conf["directory"]):
            raise ValueError("Supplied images directory %s does not exist." %
                             (conf["directory"]))
...
def load_images_labels(images_comp, labels_comp, classes_comp):
    ...
    if images_dict["directory"]:
        if labels_dict["files"] or labels_dict["file_list"]:
            raise ValueError(
                "Image directory only supported with label directory.")
        if labels_dict["directory"]:
            ...
            if os.path.realpath(labels_dict["directory"]).startswith(
                ...
                images = [
                    img for img in images if not img.endswith(label_extension)
                ]

    pre = images_comp.preprocess_function()
    imageset = ImageSet(images, images_dict["type"], pre,
                        images_dict["nodata_value"])

    if ((labels_dict["files"] is None) and (labels_dict["file_list"] is None)
            and (labels_dict["directory"] is None)):
        return (imageset, None)

    labels = __find_images(labels_dict, images, images_dict)

    if len(labels) != len(images):
        raise ValueError("%d images found, but %d labels found." %
    ...
    return (
        ...
        ImageSet(
            ...
            labels_dict["type"],
            class_shift,
            len(classes_comp) if labels_nodata is not None else None,
        ),
    )
...
class ImagePreprocessConfig(DeltaConfigComponent):
    ...
    def _load_dict(self, d, base_dir):
        ...
        for func in d:
            if isinstance(func, str):
                ...
                assert isinstance(
                    func, dict), "preprocess items must be strings or dicts"
    ...
    def function(self, image_type):
        ...
        for (name, args) in self._functions:
            ...
            def helper(cur, prev):
                return lambda data, roi, bands: cur(prev(data, roi, bands),
                                                    roi, bands)
...
class ImageSetConfig(DeltaConfigComponent):
    ...
    def __init__(self, name=None):
        ...
        self.register_field("files", list, None, _validate_paths,
                            "List of image files.")
        self.register_field("file_list", str, None, validate_path,
                            "File listing image files.")
        self.register_field("directory", str, None, validate_path,
                            "Directory of image files.")
        self.register_field("extension", str, None, None,
                            "Image file extension.")
        self.register_field("nodata_value", (float, int), None, None,
                            "Value of pixels to ignore.")
    ...
    def preprocess_function(self):
        ...
        """
        return self._components["preprocess"].function(
            self._config_dict["type"])
    ...
    def parse_args(self, options):
        ...
        if hasattr(options, self._name) and getattr(options,
...
class ClassesConfig(DeltaConfigComponent):
        ...
        if isinstance(d, int):
            ...
            for (i, c) in enumerate(d):
                if isinstance(c, int):  # just pixel value
                    ...
                    keys = c.keys()
                    assert len(
                        keys) == 1, "Dict should have name of pixel value."
                    k = next(iter(keys))
                    ...
                    self._classes.append(
                        LabelClass(
                            k,
                            str(inner_dict.get("name")),
                            inner_dict.get("color"),
                            inner_dict.get("weight"),
                        ))
        elif isinstance(d, dict):
            for k in d:
                assert isinstance(k, int), "Class label value must be int."
                self._classes.append(
                    LabelClass(k, str(d[k].get("name")), d[k].get("color"),
                               d[k].get("weight")))
        else:
            raise ValueError(
                "Expected classes to be an int or list in config, was " +
                str(d))
    ...
    def weights(self):
        ...
        assert len(weights) == len(
            self._classes
        ), "For class weights, either all or none must be specified."
        return weights
...
class DatasetConfig(DeltaConfigComponent):
    ...
    def __init__(self):
        ...
        self.register_component(ImageSetConfig("image"), "images",
                                "__image_comp")
        self.register_component(ImageSetConfig("label"), "labels",
                                "__label_comp")
        self.__images = None
        self.__labels = None
    ...
    def images(self) -> ImageSet:
        ...
        if self.__images is None:
            (self.__images, self.__labels) = load_images_labels(
                self._components["images"],
...
class CacheConfig(DeltaConfigComponent):
    ...
    def __init__(self):
        super().__init__()
        self.register_field("dir", str, None, validate_path,
                            "Cache directory.")
        self.register_field("limit", int, None, validate_positive,
    ...
    def manager(self) -> disk_folder_cache.DiskCache:
        ...
        if self._cache_manager is None:
            ...
            self._cache_manager = disk_folder_cache.DiskCache(
                cdir, self._config_dict["limit"])
...
def _validate_tile_size(size, _):
    ...
    assert isinstance(size[0], int) and isinstance(
        size[1], int), "Size must be integer."
...
class IOConfig(DeltaConfigComponent):
    ...
    def __init__(self):
        super().__init__("IO")
        self.register_field("threads", int, None, None,
